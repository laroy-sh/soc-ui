...
function renderKPIs() {
    ...
    animateValue('e8-complianceScore', 0, complianceScore, 1000, '%');
    animateValue('e8-effectiveCount', 0, effective, 800);
    animateValue('e8-automatedCount', 0, automated, 800);
    animateValue('e8-actionRequired', 0, actionRequired, 800);

    // Animate circular progress
    setTimeout(() => {
        const progress = document.getElementById('e8-scoreProgress');
        if (progress) {
...
function updateLastUpdated() {
    const element = document.getElementById('e8-lastUpdated');
    if (element) {
        const now = new Date();
...
function updateSentinelSync() {
    const element = document.getElementById('e8-sentinelLastSync');
    if (element) {
        const now = new Date();
        element.textContent = now.toLocaleDateString('en-AU', {
            year: 'numeric',
...
function renderTrendChart(selectedQuarter = 3) {
    const container = document.getElementById('e8-trendChart');
    if (!container) return;
...
function renderStrategyChart() {
    const container = document.getElementById('e8-strategyChart');
    if (!container) return;
...
function renderStatusDonut() {
    const container = document.getElementById('e8-statusDonut');
    const legend = document.getElementById('e8-statusLegend');
...
function renderEvidenceBars() {
    const container = document.getElementById('e8-evidenceBars');
    if (!container) return;
...
function renderControlsTable(filteredControls = null) {
    const tbody = document.getElementById('e8-controlsTableBody');
    if (!tbody) return;
    ...
    tbody.innerHTML = controls.map(control => `
        <tr data-id="${control.id}">
            ...
            <td>
                <button class="action-btn" onclick="e8ShowControlDetails('${control.id}')">View</button>
            </td>
        </tr>
    `).join('');
}
...
function renderInsights() {
    const container = document.getElementById('e8-insightsGrid');
    if (!container) return;
...
function showControlDetails(controlId) {
    ...
    const modal = document.getElementById('e8-detailModal');
    const overlay = document.getElementById('e8-modalOverlay');
    const title = document.getElementById('e8-modalTitle');
    const body = document.getElementById('e8-modalBody');
...
function setupEventListeners() {
    // Modal close
    document.getElementById('e8-modalClose')?.addEventListener('click', closeModal);
    document.getElementById('e8-modalOverlay')?.addEventListener('click', (e) => {
        if (e.target.id === 'e8-modalOverlay') closeModal();
    });
    ...
    // Quarter selector
    document.getElementById('e8-quarterSelector')?.addEventListener('change', (e) => {
        const quarterIndex = parseInt(e.target.value);
        renderTrendChart(quarterIndex);
    });

    // Refresh button
    document.getElementById('e8-refreshBtn')?.addEventListener('click', () => {
        // Simulate refresh with loading animation
        const btn = document.getElementById('e8-refreshBtn');
    ...
    // Filters
    document.getElementById('e8-statusFilter')?.addEventListener('change', applyFilters);
    document.getElementById('e8-testFilter')?.addEventListener('change', applyFilters);
    document.getElementById('e8-searchInput')?.addEventListener('input', applyFilters);
}

function applyFilters() {
    const statusFilter = document.getElementById('e8-statusFilter')?.value || 'all';
    const testFilter = document.getElementById('e8-testFilter')?.value || 'all';
    const searchTerm = document.getElementById('e8-searchInput')?.value.toLowerCase() || '';
...
function animateOnScroll() {
    ...
    document.querySelectorAll('.e8 .card, .e8 .kpi-card, .e8 .insight-item').forEach(el => {
        observer.observe(el);
    });
}
...
window.e8ShowControlDetails = showControlDetails;
})();
